...
## 3. 핵심 인터랙션 (Core Interactions: Caring Loop)

### 3.1 Caring Trigger (돌보기 발견)
*   **Visual:** 정원 뷰(`GardenWidget`)에서 특정 식물(이전에 심은 감정) 위에 **물방울 아이콘(💧)**이 떠올라 반짝입니다.
*   **Condition:** 해당 감정이 심어진 지 **4시간** 이상 경과되었을 때 (Pending 상태가 Ready로 전환).
*   **Action:** 사용자가 물방울 아이콘(또는 해당 식물)을 탭합니다.
*   **Transition:** **Caring Screen** (`spec/50-ui/03-caring-flow.md`)으로 부드럽게 화면 전환 (Hero Animation).

### 3.2 묻고 답하기 (Self-Talk)
*   **Concept:** 식물은 말을 하지 않지만, 질문을 던집니다.
*   **Action:** 식물을 탭하면, 식물이 현재 상태(Visual)에 기반한 **상태 메시지**나 **짧은 위로**를 띄웁니다.
...
*   **Effects:**
    ...
    *   **Sound:** 바람 소리, 물소리 등 자연의 소리가 레이어링됩니다.
