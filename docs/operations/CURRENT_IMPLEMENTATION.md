...
- 단, `/dev`, `/seeding`, `/history`, `/sleep`, `/settings`, `/login`은 디버그/개발 편의상 온보딩 없이도 열 수 있음
...
로그아웃은 실제 인증이 아니라 세션 모델을 초기화한 뒤 `/login`으로 이동하는 수준이다.
...
하지만 실제 메인 UI는 주로 `SleepNotifier -> LocalDbService` 경로를 타고 있다.

즉, 수면 저장은 현재 **Isar 직접 저장 + Hive repository 구조가 동시에 존재하는 하이브리드 상태**다.
...
- 수면은 `Isar`와 `Hive`가 동시에 남아 있음

### 14.2 문서와 코드의 불일치

- `spec/` 문서 상당수가 `Mind-Gardener` 시대 기준
...
8. 필요 시 `docs/archive/PROJECT_DOCUMENTATION_LEGACY.md`와 `spec/`를 레거시 참고용으로만 확인
...
- 수면 저장 구조, 레거시 모듈, 오래된 문서는 아직 정리 대상이다
